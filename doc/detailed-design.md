...
| `/api/projects/{id}/documents/*` | document-service | 文档相关（上传/列表/详情/下载/删除/重索引） |
| `/api/projects/{id}/search` | kb-service | 项目知识库搜索 |
| `/api/institutional/search` | kb-service | 机构知识库搜索 |
| `/api/projects/{id}/tasks/*` | orchestration-service | 任务管理（CRUD） |
| `/api/tasks/{id}/export` | output-service | 文件导出/下载（返回二进制文件流） |
| `/api/tasks/{id}/*` | orchestration-service | 任务状态/输出/取消/重试 |
...
**响应** (200):
- `Content-Type`: 根据文档格式自动设置（pdf→application/pdf, docx→application/vnd.openxmlformats-officedocument.wordprocessingml.document 等）
- `Content-Disposition`: `attachment; filename*=UTF-8''<RFC 5987 编码的文件名>`，支持中文文件名

**错误**:
- `404 DOC_NOT_FOUND`: 文档不存在或不属于该项目
- `404 FILE_NOT_FOUND`: 存储路径为空
- `500 DOWNLOAD_FAILED`: MinIO 下载失败
...
返回文件流（由 API Gateway 路由至 output-service）：
- `Content-Type`: `application/vnd.openxmlformats-officedocument.wordprocessingml.document`（根据 format 参数自动映射）
- `Content-Disposition`: `attachment; filename*=UTF-8''<RFC 5987 编码的文件名>`，支持中文文件名
- 支持格式：`md`, `docx`, `xlsx`, `pptx`

**错误**:
- `404 NOT_FOUND`: 指定格式的输出文件不存在
- `400`: 不支持的输出格式
...
*文档版本：v1.3 | 日期：2026-05-29 | 基于概要设计文档 v1.0*
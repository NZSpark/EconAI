...
**建议**：生产环境配置 `CORS_ORIGINS=["https://policyai.institution.cn"]`，开发环境保留 `*`。
...
| 位置 | 默认值 |
|------|--------|
| `shared/config.py:33` | `postgres_password: str = "policyai_secret_change_me"` |
| `shared/config.py:41` | `jwt_secret: str = "policyai_jwt_secret_change_me_min_32_chars"` |
...
**建议**：创建独立的 `policyai_audit` 用户专用于写入审计日志，应用用户仅 SELECT 权限。
...
**建议**：创建共享 base image（`policyai-base`），各服务 Dockerfile 只需 `FROM policyai-base` + 复制代码。

...
子 Agent 或主控 Agent 可能因 provider（Claude API 等）的速率限制而被中断。遇到此类情况时，按以下规则处理：

1. **识别限流中断**：Agent 返回结果中明确提示 rate limit / quota exceeded / 429 等限流错误
2. **等待 30 分钟**：遇到限流后，**必须等待 30 分钟**再重试，不可立即重试或缩短等待时间
3. **保留已完成的进度**：限流中断时，检查是否有已完成并提交的代码（通过 `git log`），已提交的工作无需重做
4. **从断点继续**：等待结束后，重新启动该 Agent，并在 prompt 中告知已完成的工作和剩余待做的工作
5. **主控层重试**：如果是主控 Agent 自身遇到限流（例如调度或汇总时），同样等待 30 分钟后继续当前 Wave
6. **重试次数**：单个模块最多重试 3 次；超过 3 次仍失败，标记该模块为「需人工介入」并继续其他模块
